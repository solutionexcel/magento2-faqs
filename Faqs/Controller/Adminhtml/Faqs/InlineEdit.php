        ...
        if ($this->getRequest()->getParam('isAjax')) {
            ...
            if (!count($postItems)) {
                ...
                foreach (array_keys($postItems) as $modelid) {
                    /** @var \Solutionexcel\Faqs\Model\Faqs $model */
                    $model = $this->_objectManager->create('Solutionexcel\Faqs\Model\Faqs')->load($modelid);
                    try {
                        $model->setData(array_merge($model->getData(), $postItems[$modelid]));
                        $model->save();
                    } catch (\Exception $e) {
                        $messages[] = "[Faqs ID: {$modelid}]  {$e->getMessage()}";
                        $error = true;
                    }
                }
            }
